            ...
            if timed_out_users:
                ...
                for pid, username, _ in timed_out_users:
                    try:
                        ...
                        # --- ФІКС 1: СИНТАКСИС aiogram 3 (з логів) ---
                        key = StorageKey(bot_id=bot.id, chat_id=pid, user_id=pid)
                        await dp.storage.set_state(key=key, state=None)
                    except Exception as e:
...
def build_locations_keyboard(token: str, locations: list, columns: int = 3) -> InlineKeyboardMarkup:
    ...
    - callback_data: 'spy_guess_{token}_{location}' з заміною пробілів на '---'.
    """
    inline_keyboard = []
    row = []
    for loc in locations:
        safe_loc = loc.replace(' ', '---')  # Заміна пробілів для callback_data
        button = InlineKeyboardButton(text=loc, callback_data=f"spy_guess_{token}_{safe_loc}")
        row.append(button)
        if len(row) == columns:
...
async def process_spy_guess_callback(callback_query: types.CallbackQuery):
    ...
    try:
        ...
        # Шукаємо токен. Він може бути 'test_1234' або 'auto_abcd' або 'abcdefgh'
        # Локація не може містити '_'
       
        # Розбиваємо по першому '_' після префікса.
        # Все, що до - токен. Все, що після - локація.
        try:
            token, guessed_location_safe = data_str.split('_', 1)
            guessed_location = guessed_location_safe.replace('---', ' ')
        except ValueError as e:
            logger.error(f"CRITICAL: Cannot parse spy_guess callback data: {callback_query.data} | Error: {e}")
